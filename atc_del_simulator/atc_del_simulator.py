...
def get_flight_plans(ads_config: AdsConfig, origin, number, traffic_type, details):
    ...
    for departure in departures:
        if len(flight_plans) >= number:
            break
        if details:
            if 'operator_icao' in departure and departure['operator_icao'] and not departure['operator_icao'] in operator_cache:
            ...
            if 'aircraft_type' in departure and departure['aircraft_type'] and not departure['aircraft_type'] in aircraft_cache:
        ...
        aircraft_type = departure['aircraft_type'].strip() if 'aircraft_type' in departure and departure['aircraft_type'] else ''
        aircraft_details = aircraft_cache[aircraft_type] if 'aircraft_type' in departure and aircraft_type and aircraft_type in aircraft_cache else ''
        origin_icao = departure['origin']['code_icao'].strip() if 'origin' in departure and 'code_icao' in departure['origin'] and departure['origin']['code_icao'] else ''
        origin_name = departure['origin']['name'].strip() if 'origin' in departure and 'name' in departure['origin'] and departure['origin']['name'] else ''
        origin_raw_metar = origin_metar
        destination_icao = departure['destination']['code_icao'].strip() if 'destination' in departure and 'code_icao' in departure['destination'] and departure['destination']['code_icao'] else ''
        destination_name = departure['destination']['name'].strip() if 'destination' in departure and 'name' in departure['destination'] and departure['destination']['name'] else ''
        operator_icao = departure['operator_icao'].strip() if 'operator_icao' in departure and departure['operator_icao'] else ''
        operator_details = operator_cache[operator_icao] if 'operator_icao' in departure and operator_icao and operator_icao in operator_cache else ''
